	...
	<div class="tab-content">
		<div class="tab-pane active" id="base">
			...
			<div class="form-group row <?if (!empty($model->errors['rubric_id'])) :?> error<?endif;?>">
				...
				<div class="col-sm-10">
					<?=$form->dropDownList($model,'rubric_id', CHtml::listData(PhotoStoryRubric::model()->findAll(array('order'=>'t.title ASC')), 'id', 'title'), array('empty'=>'Выберите рубрику', 'class' => 'form-control')); ?>
				</div>
			</div>
			...
			<?if (Yii::app()->getUser()->isAdmin()) :?>
				...
				<div class="form-group row <?if (!empty($model->errors['show_in_sidebar'])) :?> error<?endif;?>">
					<?=$form->labelEx($model,'show_in_sidebar', array('class'=>'col-sm-2 control-label')); ?>
					<div class="col-sm-10">
						<?=$form->checkBox($model,'show_in_sidebar'); ?>
					</div>
				</div>
			<?endif;?>
		...
		<div class="tab-pane" id="images">
			...
			<div class="form-group row <?if (!empty($model->errors['image'])) :?> error<?endif;?>">
				...
				<div class="col-sm-10">
					<?$this->widget('ImageUploadWidget', array('attribute'=>'image', 'model' => $model, 'type'=>'photoStory_admin/form')); ?>
				</div>
			</div>
            ...
            <div class="form-group row <?if (!empty($model->errors['image_top_3'])) :?> error<?endif;?>">
                ...
                <div class="col-sm-10">
                    <?$this->widget('ImageCropWidget', array('attribute'=>'image_top_3', 'model' => $model, 'cropFrom' => 'image', 'cropFromType' => 'photoStory/detail', 'minWidth' => 630, 'minHeight' => 391, 'previewType' => 'photoStory/topIndex')); ?>
                </div>
            </div>
            ...
            <div class="form-group row <?if (!empty($model->errors['image_top_1'])) :?> error<?endif;?>">
                ...
                <div class="col-sm-10">
                    <?$this->widget('ImageCropWidget', array('attribute'=>'image_top_1', 'model' => $model, 'cropFrom' => 'image', 'cropFromType' => 'photoStory/detail', 'minWidth' => 300, 'minHeight' => 189, 'previewType' => 'photoStory/top')); ?>
                </div>
            </div>


			<div class="form-group row <?if (!empty($model->errors['image_sidebar'])) :?> error<?endif;?>">
				<?=$form->labelEx($model,'image_sidebar', array('class'=>'col-sm-2 control-label')); ?>
				<div class="col-sm-10">
					<?$this->widget('ImageUploadWidget', array('attribute'=>'image_sidebar', 'model' => $model, 'type'=>'photoStory_admin/form')); ?>
				</div>
			</div>
		</div>
