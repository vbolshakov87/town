...
class EJuiDateTimePicker extends CJuiDatePicker
{
    ...
    public function run()
    {
        if ($this->mode == 'date') {
	        ...
	        if ($this->hasModel() && $this->model->{$this->attribute} != 'Не выбрано' && $this->model->{$this->attribute} == intval($this->model->{$this->attribute})) {
            ...
            if ($this->language != 'en') {
	            ...
	            if (!empty($this->calendarName))
		            $beforeText = $this->calendarName. ' = ';

                $js = "$beforeText $('#{$id}').{$this->mode}picker($.extend($.datepicker.regional['{$this->language}'], {$options}));";
            }
